...
@section('content')
    ...
    <div class="h-screen relative flex justify-center">
        <form action="{{ route('type.post') }}" class="my-auto" method="POST">
            @csrf
            <div class="flex flex-col bg-[#997b70] p-12 rounded-2xl">
                <button type="submit" name="type" value="kerja_praktek"
                    class="w-52 bg-[#f6f2ef] text-[#997b70] py-6 px-3 rounded-2xl mb-3 text-xl hover:bg-[#ccab90]">Kerja
                    Praktek</button>
                <button type="submit" name="type" value="tugas_akhir"
                    class="w-52 bg-[#f6f2ef] text-[#997b70] py-6 px-3 rounded-2xl mt-3 text-xl hover:bg-[#ccab90]">Tugas
                    Akhir</button>
            </div>
            <a href="#">
